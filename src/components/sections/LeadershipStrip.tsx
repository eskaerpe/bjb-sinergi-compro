...
// Ensure standard institutional ordering: Direktur Utama, Direktur, Komisaris
...
const ROLE_BADGES: Record<string, { label: string; badgeClass: string }> = {
  'Direktur Utama': {
    ...
    badgeClass: 'bg-coral-50 text-coral-600 border-coral-200'
  },
  'Direktur': {
    label: 'Pengembangan & Inovasi',
    badgeClass: 'bg-brandBlue-50 text-brandBlue-600 border-brandBlue-200'
  },
  'Komisaris': {
          ...
          {leaders.map((leader) => {
            ...
            return (
              <div
                key={leader.id}
                className={cn(
                  'group relative bg-white rounded-3xl p-6 sm:p-7 border transition-all duration-300 flex flex-col justify-between hover:-translate-y-1',
                  isDirut
                    ? 'border-brandBlue-300 ring-1 ring-brandBlue-500/20 shadow-card hover:shadow-card-hover'
                    : 'border-border-subtle hover:border-brandBlue-300 shadow-card hover:shadow-card-hover'
                )}
              >
                {/* Dirut Highlight Indicator */}
                {isDirut && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-brandBlue-600 text-white text-[10px] font-extrabold uppercase tracking-wider rounded-full shadow-xs">
                    Eksekutif Utama
                  </div>
                )}
                ...
                <div className="space-y-5">
                  {/* Photo Frame */}
                  <div className="relative rounded-2xl overflow-hidden border border-navy-100 bg-navy-900 h-60 sm:h-64 w-full">
                    <img
                    ...
                    {/* Floating Reputation Role Tag on Image */}
                    <div className="absolute bottom-3 left-3 right-3">
                      <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-white bg-navy-950/85 backdrop-blur-xs px-2.5 py-1 rounded-lg border border-white/20">
                  ...
                  <div className="space-y-2">
                    ...
                    <h3 className="text-lg font-extrabold text-navy-900 leading-snug group-hover:text-brandBlue-600 transition-colors">
                      {leader.name}
                    </h3>

                    <p className="text-xs sm:text-[13px] text-navy-700 leading-relaxed line-clamp-3">
                      {leader.bio}
                    </p>
                  </div>
                </div>
                ...
                {/* Footer Micro Tag */}
                <div className="mt-5 pt-3.5 border-t border-border-subtle/80 flex items-center justify-between text-xs text-navy-500">
